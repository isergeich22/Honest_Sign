...
app.get('/sber', async function(req, res){
        ...
        let response = await fetch('https://api.megamarket.tech/api/market/v1/orderService/order/search', {
            ...
            body: JSON.stringify({
                ...
                "data": {
                    ...
                    "dateFrom" : "2023-08-24T00:00:00Z",
                    "dateTo" : "2023-08-27T00:00:00Z",
                    "count": 10,
                    "statuses" : [
                        "CONFIRMED"
                    ]
    ...
    for(let i = 0; i < products.length; i++) {
        ...
        productsNames.push(result.result.name)
    ...
    for(let i = 0; i < productsNames.length; i++) {
        // console.log(nat_cat.indexOf(productsNames[i]))
        if(nat_cat.indexOf(productsNames[i]) < 0 && moderation_products.indexOf(productsNames[i]) < 0 && newProducts.indexOf(productsNames[i]) < 0) {
            newProducts.push(productsNames[i])
        }
    }
    ...
    async function createImport(array) {
        ...
        for(i = 0; i < array.length; i++) {
            let size = ''            
                ...
                ws.getCell(`C${cellNumber}`).value = 'Ивановский текстиль'
                ws.getCell(`D${cellNumber}`).value = 'Артикул'
                let index = productsNames.indexOf(array[i])
                ws.getCell(`E${cellNumber}`).value = products[index]
                for(let c = 0; c < colors.length; c++) {
                    str = colors[c].toLowerCase()
                    elem = array[i].toLowerCase()
                    if(elem.indexOf(str) >= 0) {
        ...
        month < 10 ? filePath = `./public/sber/IMPORT_TNVED_6302_${date_ob.getDate()}_0${month}_sber` : filePath = `./public/sber/IMPORT_TNVED_6302_${date_ob.getDate()}_0${month}_sber`
